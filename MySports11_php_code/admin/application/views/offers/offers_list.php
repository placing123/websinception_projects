<header class="panel-heading">
  <h1 class="h4"><?php echo ucfirst($this->ModuleData['ModuleTitle']);?></h1>
</header>
...
<div class="panel-body" ng-controller="PageController"><!-- Body -->
	...
	<div class="clearfix mt-2 mb-2">
		...
		<div class="float-right"><!-- ng-if="filterData.CategoryTypes.length>1" -->
			...
			<button class="btn btn-success btn-sm ml-1" ng-click="loadFormAdd();">Add Offers</button>
		</div>
	</div>
	<!-- Top container/ -->
		...
		<table class="table table-striped table-condensed table-hover table-sortable" ng-if="data.dataList.length">
			...
			<thead>
				<tr>
					...
					<th>Offer Type</th>
					<th>Offer Percent</th>
					<th>User Selection Type</th>
					<th>No Of RandomUsers</th>
					<th>Offer DateTime </th>
			...
			<tbody id="tabledivbody">
				...
				<tr scope="row" ng-repeat="(key, row) in data.dataList">
					...
					<td>{{row.OfferName}}</td>
 				    <td>{{row.OfferType}}</td>
					<td>{{row.OfferPercent}}</td>
					<td>{{row.UserSelectionType}}</td>
 				    <td>{{row.NoOfRandomUsers}}</td>
					<td>{{row.OfferDateTime}}</td>
					<!-- <td class="text-center">
						<div class="dropdown action_toggle">
							...
							<div class="dropdown-menu dropdown-menu-left">
								...
								<a class="dropdown-item" href="" ng-click="loadFormDelete(key, row.ID)">Delete</a>
							</div>
						</div>
					</td> -->
				</tr>
