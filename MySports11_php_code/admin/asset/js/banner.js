app.controller('PageController', function ($scope, $http,$timeout,$window){
    ...
    $scope.IsActive =  function(MediaGUID,Status){
        if ($window.confirm("Do you really want to change status?")) {
            var data = 'SessionKey=' + SessionKey + '&MediaGUID=' + MediaGUID + '&IsActive=' + Status;
            
            $http.post(API_URL + 'admin/config/isActiveStatus', data, contentType).then(function (response) {
                var response = response.data;
                if (response.ResponseCode == 200) { /* success case */
                    alertify.success(response.Message);
                    $scope.reloadPage();
    ...
    $scope.deleteBannerData =  function(EntityGUID){
        if ($window.confirm("Do you really want to delete?")) {
            var data = 'SessionKey=' + SessionKey + '&EntityGUID=' + EntityGUID;
            $http.post(API_URL + 'admin/entity/BannerDeleteAdmin', data, contentType).then(function(response) {
                var response = response.data;
                if (response.ResponseCode == 200) { /* success case */
                    alertify.success(response.Message);
                    $scope.reloadPage();
