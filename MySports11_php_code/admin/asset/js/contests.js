app.controller('PageController', function ($scope, $http, $timeout, $rootScope) {
    ...
    $scope.getList = function ()
    {
        ...
        var WinningType = "Paid Join Contest";
        ...
        var data = 'Keyword=' + myJSON + '&SessionKey=' + SessionKey + '&WinningType=' + WinningType + '&GUID=' + MatchGUID + '&PageNo=' + $scope.data.pageNo + '&PageSize=' + $scope.data.pageSize + '&Params=MatchGUID,IsVirtualUserJoined,IsConfirm,VirtualUserJoinedPercentage,GameType,GameTimeLive,AdminPercent,Privacy,UserInvitationCode,IsPaid,WinningAmount,ContestSize,EntryFee,NoOfWinners,EntryType,TeamNameLocal,TeamNameVisitor,Status,CustomizeWinning,ContestType,MatchStartDateTime,TotalJoined,TotalAmountReceived,TotalWinningAmount,CashBonusContribution,UserJoinLimit&Privacy=No&OrderByList=Yes&' + $('#filterForm1').serialize() + "&" + $('#filterForm').serialize() + "&Status=" + $rootScope.Status;
        console.log(data);
    ...
    $scope.getMatches = function (SeriesGUID, Status) {
        $scope.MatchData = {};
        //&StatusID=1
        var data = 'SeriesGUID=' + SeriesGUID + '&MatchTypeByApi=Real&Params=MatchNo,MatchStartDateTime,TeamNameLocal,TeamNameVisitor&OrderBy=MatchStartDateTime&Sequence=ASC&Status=' + Status;
    ...
    $(document).on('click', '#all_matches', function () {
        $('select.chosen-select1 option').prop('selected', true);
        $('select.chosen-select1 option[value=""]').prop('selected', false);
        $('select.chosen-select1').chosen({width: '100%', "disable_search_threshold": 8}).trigger("chosen:updated");
    });

    $(document).on('click', '#clear_all', function () {
        $('select.chosen-select1 option').prop('selected', false);
        $('select.chosen-select1').chosen({width: '100%', "disable_search_threshold": 8}).trigger("chosen:updated");
    });
    ...
    $scope.addData = function ()
    {
        ...
        var data = 'SessionKey=' + SessionKey + '&' + $("form[name='add_form']").serialize() + '&CustomizeWinning=' + customWinings;
        $http.post(API_URL + 'admin/contest/add', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                alertify.success(response.Message);
                $scope.applyFilter($rootScope.Status);
    ...
    $scope.editData = function ()
    {
        ...
        inputData.WinningType = "Paid Join Contest";
            ...
            for (var $i = 0; $i < $scope.custom.choices.length; $i++)
            {
                for (var i in $scope.data.categoryList) {
                    if ($scope.data.categoryList[i].CategoryGUID == $scope.custom.choices[$i].CategoryGUID) {
                        var Cat_url = $scope.data.categoryList[i].Media.Records[0].MediaRealURL;
    ...
    $scope.loadContestJoinedUser = function (Position, ContestGUID, MatchGUID)
    {

        $scope.data.Position = Position;
        $scope.templateURLEdit = PATH_TEMPLATE + module + '/joinedContest_form.htm?' + Math.random();
        $scope.data.pageLoading = true;
        $http.post(API_URL + 'contest/getJoinedContestsUsers', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&MatchGUID=' + MatchGUID + '&Params=UserTypeID,Email,PhoneNumber,UserTeamName,Email,PhoneNumber,TotalPoints,UserWinningAmount,FirstName,Username,UserGUID,UserTeamPlayers,UserTeamID,UserRank,UserTypeID', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
                $scope.formData = response.Data;
        ...
        $http.post(API_URL + 'contest/getContest', 'SessionKey=' + SessionKey + '&ContestGUID=' + ContestGUID + '&MatchGUID=' + MatchGUID + '&Params=AdminPercent,IsAutoCreate,Privacy,UserInvitationCode,IsPaid,WinningAmount,ContestSize,EntryFee,NoOfWinners,EntryType,SeriesID,MatchID,SeriesGUID,TeamNameLocal,TeamNameVisitor,SeriesName,CustomizeWinning,ContestType,CashBonusContribution,UserJoinLimit,ContestFormat,IsConfirm,ShowJoinedContest,TotalJoined', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.pageLoading = false;
   ...
   $scope.delete_all = function ()
    {
        ...
        if (r) {
            var data = 'SessionKey=' + SessionKey + '&' + $("#Delete_all").serialize();
            console.log(data);
            $http.post(API_URL + 'admin/contest/CanacleContest', data, contentType).then(function (response) {
                var response = response.data;
                if (response.ResponseCode == 200) { /* success case */
                    alertify.success(response.Message);
