app.controller('PageController', function ($scope, $http, $timeout) {
    ...
    $scope.LoadUserList = function (Type)
    {
        if (Type == 'Today') {
            ...
            window.open(BASE_URL + 'user?Type=Subscribe');
    ...
    /*Wallet List*/
    $scope.getWalletBalnaceList = function ()
    {
        if ($scope.data.listLoading || $scope.data.noRecords)
            return;
        $scope.data.listLoading = true;
        var data = 'SessionKey=' + SessionKey;
        $http.post(API_URL + 'utilities/dashboardWalletStatics', data, contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
                $scope.data.dataList = response.Data;
                $scope.data.pageNo++;
