app.controller('PageController', function ($scope, $http,$timeout){
    ...
    $scope.Playing11Notification = function (MatchGUID)
    {
        alertify.confirm('Are you sure you want sent notifiaction?', function() {
            ...
            $http.post(API_URL + 'admin/matches/playing11NotificationAdmin', data, contentType).then(function(response) {
                var response = response.data;
    ...
    $scope.getList = function ()
    {
        ...
        var data = 'SessionKey='+SessionKey+'&MatchTypeByApi=Virtual&Filter=AddDays'+'&MatchTypeByApi=Virtual'+'&OrderBy=' + $scope.data.OrderBy + '&Sequence=' + $scope.data.Sequence +'&OrderByToday=Yes&Params='+'SeriesName,MatchDisplay,TotalPlayerCountMatch,IsEdited,MatchType,MatchNo,MatchStartDateTime,TeamNameLocal,TeamNameVisitor,TeamNameShortLocal,IsPlayingXINotificationSent,TeamNameShortVisitor,TeamFlagLocal,TeamFlagVisitor,MatchLocation,Status&PageNo='+$scope.data.pageNo+'&PageSize='+$scope.data.pageSize+'&'+$('#filterForm').serialize()+'&'+$('#filterForm1').serialize()+'&StatusID=1';
    ...
    $scope.ExportExcelMatches = function () {
        ...
        $http.post(API_URL + 'admin/matches/getMatchesExport', 'SessionKey=' + SessionKey +'&'+$('#filterForm1').serialize() + '&' + $('#filterForm').serialize()+'&StatusID=1', contentType).then(function (response) {
            var response = response.data;
            if (response.ResponseCode == 200) { /* success case */
    ...
    $scope.editStatusMatchDisplay = function (StatusID, MatchGUID)
    {
        $scope.editDataLoading = true;
            ...
            $http.post(API_URL + 'admin/matches/editStatusMatchDisplay', data, contentType).then(function (response) {
                var response = response.data;
                if (response.ResponseCode == 200) { /* success case */
                    alertify.success(response.Message);
                    // $scope.data.dataList[$scope.data.Position] = response.Data;
    ...
    $scope.makeLive = function (MatchGUID)
    {
        alertify.confirm('Are you sure you want go live?', function() {
            var data = 'SessionKey=' + SessionKey + '&MatchGUID=' + MatchGUID + '&Status=Running';
                $http.post(API_URL + 'admin/matches/goLive', data, contentType).then(function(response) {
                var response = response.data;
                if (response.ResponseCode == 200) { /* success case */
                    alertify.success(response.Message);
                    window.location.reload();
