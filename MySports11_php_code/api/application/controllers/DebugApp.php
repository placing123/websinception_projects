...
class DebugApp extends API_Controller {
    ...
    public function createUsers_get() {
        ...
        for ($j = 0; $j < 10; $j++) {
            ...
            $email = "";
            //for ($i = 1; $i <= $ulen; $i++) {
            $email .= substr($char, mt_rand(0, strlen($char)), 1) . uniqid();
            // }
            $email .= "@";
            $email .= "gmail";
            $email .= ".";
    ...
    public function createTeam_get() {
        ...
        $AllUsers = $this->Users_model->getUsers('UserID', array('UserTypeID' => 3), true, 1, 5000);
        if (!empty($AllUsers)) {
            $Match = $this->Sports_model->getMatches('SeriesID,MatchID,TeamIDLocal,TeamIDVisitor', array('StatusID' => array(1, 2), 'MatchID' => 18229), False, 1, 1);
            if (!empty($Match)) {
    ...
    public function autoJoinContest_get() {
        $Match = $this->Sports_model->getMatches('SeriesID,MatchID,TeamIDLocal,TeamIDVisitor,MatchStartDateTime', array('StatusID' => array(1, 2), 'MatchID' => 18229), False, 1, 1);
        if (!empty($Match)) {
            $MatchID = $Match['MatchID'];
            $SeriesID = $Match['SeriesID'];
            $ContestData = $this->Contest_model->getContests("ContestID,ContestSize,TotalJoined,CustomizeWinning,EntryFee,EntryType,IsPaid,CashBonusContribution", array('MatchID' => $MatchID,
                'ContestType' => "Normal",
                ...
                foreach ($ContestData['Data']['Records'] as $contest) {
                    ...
                    if (!empty($AllUsers)) {
                        foreach ($AllUsers['Data']['Records'] as $users) {
                            ...
                            if (!empty($UserTeams)) {
                                $Input = array(
                                    'IsPaid' => $contest['IsPaid'],
                                    'EntryFee' => $contest['EntryFee'],
                                    'CashBonusContribution' => $contest['CashBonusContribution'],
                                );
                                $this->Contest_model->joinContest($Input, $users['UserID'], $contest['ContestID'],$MatchID, $UserTeams['UserTeamID']);
                            }
                        }
                    }
                }
