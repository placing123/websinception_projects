...
class Users extends API_Controller_Secure {
    ...
    public function getProfile_post() {
        ...
        if ($UserData) {
            ...
            if(empty($UserData['SubscriptionEnd'])){
                $UserData['SubscriptionsStatus'] = FALSE;
            }else{
                if($UserData['SubscriptionEnd'] < date('Y-m-d H:i:s')){
                    $UserData['SubscriptionsStatus'] = FALSE;    
                }else{
                    $UserData['SubscriptionsStatus'] = TRUE;
                }
            }

            $subscription = $this->db->query("SELECT S.* FROM tbl_users_wallet W JOIN tbl_subscription S on S.ID = W.SubscriptionID WHERE W.UserID = $this->UserID AND W.SubscriptionID != '' ORDER BY W.EntryDate DESC LIMIT 1")->row();
            if(!empty($subscription)){
                $UserData['subscription'] = $subscription;
            }else{
                $UserData['subscription'] = new stdClass();
    ...
    public function updateUserInfo_post() {
        ...
        $this->Return['Data'] = $this->Users_model->getUsers('FirstName,LastName,Username,Email,ProfilePic,UserTypeID,UserTypeName', array("UserID" => $this->SessionUserID));
    ...
    function validateAlreadyRegistered($Value, $FieldValue) {
        $Query = ($FieldValue == 'Email') ? 'SELECT * FROM `tbl_users` WHERE `Email` = ' . $this->db->escape($Value) . ' OR `EmailForChange` = "' . $Value . '" LIMIT 1' : 'SELECT * FROM `tbl_users` WHERE `PhoneNumber` = ' . $this->db->escape($Value) . ' OR `PhoneNumberForChange` = "' . $Value . '" LIMIT 1';
