...
class Signin extends API_Controller {
    ...
    public function index_post() {
        ...
        $ReplaceWord=array("","","");
        $this->Post['Username'] = str_replace($FindWord,$ReplaceWord,$this->Post['Username']);
        $this->Post['Password'] = str_replace($FindWord,$ReplaceWord,$this->Post['Password']);
        ...
        if (!$UserData) {
            ...
            if (!isset($this->Post['MobileOTP']) && $UserData['UserTypeID'] == 1) {
                ...
                if (ENVIRONMENT == 'production') {
                    $this->Utility_model->sendSMS_MSG91(array(
                        'FlowID'        => LOGIN_OTP_ID,
                        'PhoneNumber'   => $UserData['PhoneNumber'],//9816126072,
                        'OTP'           =>  $Token
                    ));
            ...
            if (isset($this->Post['MobileOTP'])) {
                ...
                if(isset($this->Post['g-recaptcha-response']) && !empty($this->Post['g-recaptcha-response']))
                {
                    ...
                    if(@$this->Post['RequestType'] == "APP"){
                        $secret  = '6LeZDKcZAAAAANpbU8cRVGKSWmJWgqWq6k6KV5ov'; 
                    }else{
                        $secret  = '6LcX_ssZAAAAAACTQZW-U1iIGZvGKXGeAJUh3Ubx'; 
                    }
                    $SiteKey = '6LcX_ssZAAAAAPhAd_JaCll8pJQdj3dNVPe_1SCQ';
