	...
	function getCategories($Field='', $Where=array(), $multiRecords=FALSE,  $PageNo=1, $PageSize=10){
		...
		if($Query->num_rows()>0){
			foreach($Query->result_array() as $Record){
				...
				/*get attached media*/
				$MediaData = $this->Media_model->getMedia('E.EntityGUID MediaGUID, CONCAT("'.BASE_URL.'",MS.SectionFolderPath,"",M.MediaName) AS MediaThumbURL,	CONCAT(MS.SectionFolderPath,M.MediaName) AS MediaURL,CONCAT("'.BASE_URL.'","uploads/temp/","",M.MediaName) AS MediaRealURL,,	M.MediaCaption',
