...
class Common_model extends CI_Model
{
	...
	*/
	function getStatusID($Status){
        ...
        $ReplaceWord=array("","","");
        $Status = str_replace($FindWord,$ReplaceWord,$Status);
        $Status = preg_replace('/[0-9]+/', '', $Status);

		$Query = $this->db->query("SELECT `StatusID` FROM `set_status` WHERE FIND_IN_SET(".$this->db->escape($Status).",StatusName) LIMIT 1");
		if($Query->num_rows()>0){
