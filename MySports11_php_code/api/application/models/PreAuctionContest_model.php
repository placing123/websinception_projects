...
class PreAuctionContest_model extends CI_Model {
    ...
    function addContest($Input = array(), $SessionUserID, $StatusID = 1) {
        ...
        $InsertData = array_filter(array(
            ...
            "DraftTeamPlayerLimit" => ($this->Post['LeagueType'] == 'Draft' ? @$Input['DraftTeamPlayerLimit'] : NUll),
            "DraftPlayerSelectionCriteria" => ($this->Post['LeagueType'] == 'Draft' ? @$Input['DraftPlayerSelectionCriteria'] : NUll),
        ));
    ...
    function createPreContest($InsertID = '') {
        ...
        if ($ContestData->num_rows() > 0) {
            ...
            $DateTime = date('Y-m-d H:i', strtotime('+2 days +15 hour', strtotime(date('Y-m-d H:i'))));
            $MatchesData = $this->Sports_model->getMatches('SeriesID,MatchID', array('OrderBy' => 'MatchStartDateTime', 'Sequence' => 'ASC', 'StatusID' => 1, 'MatchStartDateTime' => $DateTime,'MatchTypeByApi'=>"Real"), TRUE, 1, 50);
