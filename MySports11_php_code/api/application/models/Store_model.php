	...
	function getCoupons($Field, $Where=array(), $multiRecords=FALSE,  $PageNo=1, $PageSize=15){
		...
		if($Query->num_rows()>0){
			foreach($Query->result_array() as $Record){
				...
				/** Get Total No of Usage Coupon Count**/
				$Record['TotalNoOfUsageCount'] = $this->db->query('SELECT COUNT(*) as TotalNoOfUsageCount FROM `tbl_users_wallet` WHERE CouponCode = "'.$Record['CouponCode'].'" AND StatusID = 5')->row()->TotalNoOfUsageCount;
