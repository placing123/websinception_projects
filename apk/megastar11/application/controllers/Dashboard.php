...
class Dashboard extends MY_Controller {
   ...
   public function __construct() 
   {
	...
	public function index()
	{
        ...
        $data['csrfName'] = '';
        $data['title'] = 'Dashboard';
        $upcoming_matches = $this->match_model->get_match_by_match_status('Fixture');
        $result_matches = $this->match_model->get_match_by_match_status_result('Result','desc');
        $Live_matches = $this->match_model->get_match_by_match_status('Live');
        
        $data['upcoming_match']  = count($upcoming_matches);
        $data['Live_matches']  = count($Live_matches);
        $data['result_matches']  = count($result_matches);
       // print_r($data['upcoming_matches']);die;
    ...
    public function match_dashboard(){
        ...
        $this->load->view('../modules/loggedin_template/header',$data);
        $this->load->view('dashboard/contest_dashboard',$data);
    ...
    public function match_leaderboard(){
        $data = array();
        $matchid = $this->uri->segment('3');
        $contestid = $this->uri->segment('4');
       // $data['match_record'] =$this->match_model->get_by_id($matchid);
        //$toss_winner_team =$data['match_record']->toss_winner_team;
        //$data['toss_winner'] =$this->team_model->get_by_unique_id($toss_winner_team);
        $data['total_teams'] =$this->match_model->total_team_joins($matchid,$contestid);
        //$data['total_team'] = count($data['total_teams']);
        $data['Topranks'] =$this->match_model->get_toprank($matchid,'');
        //$data['all_contest'] = $contests =$this->contest_model->get_contest_by_match($matchid);
        //$data['contests'] = count($contests);
