...
class Match extends MY_Controller
{
    ...
    public function index()
    {
        ...
        $match = $this->Match_model->get_fixture_match_data();
    ...
    public function update_action() 
    {
        ...
        if ($this->form_validation->run() == FALSE) {
        	...
        	$data = array(
        		...
        		'type' =>$type_name->name,
                'time' =>$this->input->post('datetime',TRUE),
                'match_date_time' =>$this->input->post('datetime',TRUE),
        		'match_status'=>$match_status->name );
    ...
    public function publishMatch()
    {
        $id = $this->input->post('id');
        $this->db->select('publish');
        $this->db->where('match_id',$id);
        $record = $this->db->get('match')->row_array();
        ...
        if($status ==1)
        {
            echo json_encode('<button style="border-radius: 6px;" type="button" class="btn btn-danger">Match published</button>');
        }    
        else if($status ==0){
            echo json_encode('<button style="border-radius: 6px;" type="button" class="btn btn-success">Not published</button>');
        } 
