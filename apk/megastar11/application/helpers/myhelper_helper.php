...
function amount_refund($data)
{
    $CI =& get_instance();
    $CI->db->insert('transection',$data);
}
...
{
	$CI =& get_instance();
    $CI->db->select('team_short_name,team_image');
    $CI->db->where('team_id',$id);
    $result = $CI->db->get('team')->row();
    return $result;
}

function get_cpatain($id)
{
    $CI =& get_instance();
    $CI->db->select('is_captain');
    $CI->db->where('id',$id);
    $CI->db->where('is_captain','1');
    $result = $CI->db->get('my_team_player')->row();
    if($result)
    {
        return '(C)';
    }    
...
function get_vicecpatain($id)
{
    $CI =& get_instance();
    $CI->db->select('is_vicecaptain');
    $CI->db->where('id',$id);
    $CI->db->where('is_vicecaptain','1');
    $result = $CI->db->get('my_team_player')->row();
    if($result)
    {
        return '(VC)';
...
function referral_code($code) {
    $CI =& get_instance();
    $CI->db->select('user_id');
    $CI->db->where('code',$code);
    $result = $CI->db->get('registration')->num_rows();
    if($result > 0)
    {
        return $result;
    }    
