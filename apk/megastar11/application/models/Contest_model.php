...
class Contest_model extends CI_Model
{
    ...
    function get_contest_id_by_match_idd($id)
    {
        $this->db->select('contest_id');
        $this->db->from($this->table);
    ...
    function get_contest_id_by_match_for_payment($id)
    {
        $this->db->select('contest_id');
        $this->db->from($this->table);
        $this->db->where('cancelled','0');
        $this->db->where('join_team !=','0');
    ...
    function get_lBoard($contest_id , $match_id)
    {
        $sql = "SELECT `user_id`, `rank`,`temprank`,`id`,`points` FROM `leaderboard` WHERE `contestid` = '".$contest_id."' AND `matchid` = '".$match_id."' ORDER BY `leaderboard`.`rank` + 0 ASC";
