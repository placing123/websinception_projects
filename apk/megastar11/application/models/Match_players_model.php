...
class Match_players_model extends CI_Model
{
    ...
    function match_type($id)
    {
        $this->db->select('title,type');
        $this->db->where('match_id',$id);
       return  $this->db->get('match')->row_array();
    }
    ...
    {
        $this->db->select('p.name,d.title,t.team_name,mp.points as ppoints,mp.selection_percent,mp.id as m_p_id, mp.matchid as match_id, mp.total_points,, mp.batting_points, mp.bolling_points, mp.fielding_points, mp.second_innings_batting, mp.second_innings_bolling, mp.second_innings_fielding');
        $this->db->where('mp.id', $id);
