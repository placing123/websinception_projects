...
class User_team_model extends CI_Model
{
    ...
    function get_team_players($id,$match_id)
    {   
        $this->db->select('my_team_player.my_team_id,my_team_player.player_id,my_team_player.designationid,my_team_player.is_captain,my_team_player.is_vicecaptain,match_players.total_points,match_players.batting_points,match_players.bolling_points,match_players.fielding_points,match_players.playing_score,match_players.second_innings_batting,match_players.second_innings_bolling,match_players.second_innings_fielding');
        $this->db->where('my_team_player.my_team_id',$id);
        $this->db->where('match_players.matchid',$match_id);
        $this->db->where('my_team_player.is_select',1);
        $this->db->from('my_team_player');
       // $this->db->join('match_players','my_team.match_id = match_players.matchid');
        $this->db->join('match_players','my_team_player.player_id = match_players.playerid');
