...
class Webservice_model extends CI_Model 
{
    ...
    {
        $this->db->select('my_match.*,match.match_date_time');
        $this->db->where('my_match.match_status',$request['status']);
        $this->db->where('my_match.user_id',$request['user_id']);
        $this->db->group_by('my_match.match_id');
        $this->db->order_by('my_match.my_match_id','desc');
        $this->db->join('match', 'match.match_id = my_match.match_id');
    ...
    function contest_list($request)
    {
        if($request['type'] =="All")
        {
            $sql =  "select *,(total_team - join_team) as difference from contest where `match_id` ='".$request['match_id']."' AND `cancelled` ='0' order by total_team asc";
    ...
    {
        $this->db->select('match_players.points as total_points,players.id,players.name,players.credit_points,players.image,players.dob,players.nationality,players.bats,players.bowls');
        $this->db->from('players');
        $this->db->where('match_players.playerid',$request['player_id']);
    ...
    {  
        if($request['my_team'] == "1")
        {
            if($request['designationid'] == "0")
            { 
                $this->db->select('my_team_player.is_vicecaptain,my_team_player.is_captain,match_players.points as player_points,match_players.selection_percent,my_team_player.id,my_team_player.my_team_id,my_team_player.player_id,my_team_player.player_id,players.image,players.teamid,my_team_player.is_select,players.credit_points,players.name,designation.title,designation.short_term,players.playing_status');
                $this->db->from('my_team_player');  
                $this->db->join('players', 'players.id = my_team_player.player_id');
                $this->db->join('match_players', 'players.id = match_players.playerid');
            ...
            {
                $this->db->select('my_team_player.id,match_players.points as player_points,match_players.selection_percent,my_team_player.my_team_id,my_team_player.player_id,my_team_player.player_id,players.teamid,players.image,my_team_player.is_select,players.credit_points,players.name,designation.title,designation.short_term,players.playing_status');
                $this->db->from('my_team_player');  
                $this->db->join('players', 'players.id = my_team_player.player_id');
                $this->db->join('designation', 'players.designationid = designation.id');
                $this->db->join('my_team', 'my_team_player.my_team_id = my_team.id');
        ...
        else
        {
            if($request['designationid'] == "0")
            { 
                $this->db->select('match_players.id,match_players.points as player_points,match_players.selection_percent,match_players.matchid,match_players.teamid,match_players.playerid,match_players.is_select,team.team_name,team.team_image,team.team_short_name ,players.credit_points,players.name,players.image,designation.title,designation.short_term,match_players.playing_status');
    ...
    function user_referral_bonus($request,$bonus_amt, $reffer)
    {
        ...
        $this->db->where('user_id',$reffer->user_id);
        $this->db->where('referral_useid',$request['user_id']);
        $this->db->where('transaction_status','SUCCESS');
        $amount = $this->db->get('transection')->row_array();
        ...
        if($amount['amount'] < 100)
        {
            ...
            else if($bonus_amt > 100 && $amount['amount'] =="0")
            {
                $bonus_amt = "100";
    ...
    // API for check bonus amount in user account by user id
    function join_contest_with_credit_and_winning_fees($request,$bonus_amt,$reffer,$credit,$remaining_amount,$my_match_id)
    {
        ...
        $my_match_amount =  array('credit_type'=>$credit,'winning_type'=>$remaining_amount,'bonus_type'=>$bonus_amt);
        $this->db->where('my_match_id',$my_match_id);
        $this->db->update('my_match',$my_match_amount);
    }


    // API for check bonus amount in user account by user id
    function join_contest_with_bonus_fees($request,$bonus_amt, $reffer,$my_match_id)
    {
        $bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $bonus_amt,
                        'type'=>'bonus_debit',
        ...
        $my_match_amount =  array('credit_type'=>$request['contest_amount'],'bonus_type'=>$bonus_amt);
        $this->db->where('my_match_id',$my_match_id);
        $this->db->update('my_match',$my_match_amount);
    }
    ...
    // API for check bonus amount in user account by user id
    function join_contest_with_bonus_fees_winning($request,$bonus_amt, $reffer,$my_match_id)
    {
        $bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $bonus_amt,
                        'type'=>'bonus_debit',
        ...
        $my_match_amount =  array('winning_type'=>$request['contest_amount'],'bonus_type'=>$bonus_amt);
        $this->db->where('my_match_id',$my_match_id);
        $this->db->update('my_match',$my_match_amount);
    }

    // API for check bonus amount in user account by user id
    function join_contest_with_credit_and_winning_fees_amount($request,$reffer,$remaining_amount,$credit,$my_match_id)
    {
        $credit_debit = array('user_id' =>$request['user_id'],
                        'amount'=> $credit,
                        'type'=>'debit',
        ...
        $my_match_amount =  array('winning_type'=>$remaining_amount,'credit_type'=>$credit);
        $this->db->where('my_match_id',$my_match_id);
        $this->db->update('my_match',$my_match_amount);
    }

    // API for check total amount in user account by user id
    function join_contest_without_bonus_fees($request,$reffer,$my_match_id)
    {
        $contest_bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $request['contest_amount'],
                        'type'=>'debit',
        ...
        $my_match_amount =  array('credit_type'=>$request['contest_amount']);
        $this->db->where('my_match_id',$my_match_id);
        $this->db->update('my_match',$my_match_amount);
    }
    ...
    function join_contest($request ,$teamno)  
    {
        ...
        if($record == "")
        {
            ...
            $my_match_id_last = $this->db->insert_id();
            if($my_match_id_last)
            {
                return $my_match_id_last;
            }
            else
            {
                return false;
    ...
    function leaderboard_contest_id($request)
    {
      ...
      $sql =    "SELECT `leaderboard`.`id`,`leaderboard`.`temprank`, `registration`.`image`, `leaderboard`.`teamid`, `leaderboard`.`rank`, `leaderboard`.`name`, `leaderboard`.`user_id`, `leaderboard`.`points`, `leaderboard`.`TeamName`
FROM registration 
JOIN leaderboard 
           ON registration.user_id = leaderboard.user_id 
WHERE contestid = '".$request['contest_id']."' 
       and leaderboard.user_id= '".$request['user_id']."' ORDER BY `leaderboard`.`rank` + 0 ASC";
...
WHERE contestid = '".$request['contest_id']."' 
    ...
    {
        $this->db->select('my_team_player.id,players.image,my_team_player.my_team_id,my_team_player.player_id,my_team_player.player_id,my_team_player.is_select,players.credit_points,players.points,players.name,players.teamid,designation.short_term,designation.title,designation.short_term');
        $this->db->from('my_team_player');  
        $this->db->join('players', 'players.id = my_team_player.player_id');
        $this->db->join('designation', 'players.designationid = designation.id');
        $this->db->join('my_team', 'my_team_player.my_team_id = my_team.id');
    ...
    {        
        $this->db->select('id,message,contest_id,message,title');
    ...
    function add_amount($request,$txn_status)
    {
        ...
        if($insert)
        {
            ...
            $this->db->where('amount',$request['amount']);
            $check = $this->db->get('deposite_bonus')->row_array();
            if( !empty($check))
            {
                $bonus_credit = array('user_id' =>$request['user_id'],
                        'amount'=>$check['percentage']*$request['amount'] / 100,
                        'type' => 'bonus',
                        'transaction_status'=>'SUCCESS',
                        'transection_mode'=>"deposit bonus",
                        'created_date'=> date('Y-m-d H:i:s'),
                    );

                $this->db->insert('transection',$bonus_credit);
            } 
    ...
    {
        $this->db->select('title,t10score,t20score,odiscore,testscore');
        $this->db->where('id',$id);
    ...
    {
        $this->db->where('match_id', $id);
        $this->db->update('my_match',array('match_status'=>"Live"));
    }
    ...
    {
        $sql =  "SELECT `leaderboard`.`user_id`, `registration`.`referral_code` as `name`, avg(leaderboard.points) as points FROM `leaderboard` JOIN `registration` ON `registration`.`user_id`= `leaderboard`.`user_id` GROUP BY `leaderboard`.`user_id` ORDER BY `points` DESC";
    ...
    function update_app(){
        ...
        $this->db->select('id,note,old_version,new_version,maintenance_status');
    ...
    function join_contest_with_bonus_fees_only($request,$reffer)
    {
        $bonus = array('user_id' =>$request['user_id'],
    ...
    function join_contest_with_bonus_fees_and_winning_fees_only($request,$reffer,$bonus,$remaning_amount)
    {
        $bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $bonus,
                        'type'=>'bonus_debit',
        ...
        $winning = array('user_id' =>$request['user_id'],
        ...
        $this->db->insert('transection',$winning);        
    }


    function join_contest_with_bonus_fees_and_credit_fees_only($request,$reffer,$bonus,$remaning_amount)
    {
        $bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $bonus,
                        'type'=>'bonus_debit',
        ...
        $credit = array('user_id' =>$request['user_id'],
                        'amount'=> $remaning_amount,
                        'type'=>'debit',
                        'referral_useid'=>$reffer->user_id,
                        'transaction_status'=> 'SUCCESS',
        ...
        $this->db->insert('transection',$credit);  

    }


    function join_contest_with_bonus_fees_credit_fee_and_winning_fees_only($request,$reffer,$bonus,$credit,$remaning_amount)
    {
        $bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $bonus,
                        'type'=>'bonus_debit',
    ...
    function join_contest_with_bonus_fees_and_winning_fees_onlysecond($request,$reffer,$bonus,$remaning_amount)
    {
        $bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $bonus,
                        'type'=>'bonus_debit',
    ...
    function join_contest_with_credit_fees($request,$reffer,$credit)
    {
        $bonus = array('user_id' =>$request['user_id'],
                        'amount'=> $credit,
                        'type'=>'debit',
    ...
    function join_contest_with_credit_and_winning_fees_only($request,$reffer,$credit,$remaning_amount)
    {
        $credit = array('user_id' =>$request['user_id'],
                        'amount'=> $credit,
                        'type'=>'debit',
    ...
    function join_contest_with_winning_fees_only($request,$reffer,$winning)
    {
        $winning = array('user_id' =>$request['user_id'],
                        'amount'=> $winning,
                        'type'=>'winning_debit',
    ...
    // API for get match status  by time 
    function get_match_status_by_type($time,$type)
    {
        $this->db->select('*');
        $this->db->where('time',$time);
        $this->db->or_where('time <=',$time);
        $this->db->where('time !=', "0000-00-00 00:00:00");
        $this->db->where('match_status',$type);
        $resp = $this->db->get('match')->result();
        if($resp !="")
        {
            return $resp;
     ...
     function get_livematch_status()
    {
        $this->db->select('match_id,unique_id');
        $this->db->where('match_status',"Live");
        $resp = $this->db->get('match')->result();
        if($resp !="")
        {
    ...
    function get_match_less_than_one_hour()
    {
        $my_date_time = date("Y-m-d H:i:s", strtotime("+1 hour"));
        $this->db->select('unique_id,title,eleven_out');
        $this->db->where('cancelled', '0');
        $this->db->where('publish', '1');
        $this->db->where('time <=', $my_date_time);
        $this->db->where('time !=', "0000-00-00 00:00:00");
        $resp = $this->db->get('match');
        if($resp->num_rows() > 0)
        {
    ...
    function  update_match_play_eleven($id)
   {
       $data = array('eleven_out'=>'1');
       ...
       $this->db->from('match');
       $this->db->where('unique_id',$id);
       $match_id = $this->db->get()->row_array();
       
       $this->db->where('match_id',$match_id['match_id']);
       $this->db->update('my_match',$data);
   }
   
   
   function get_match_player_id($match_id, $player_id)
    {
        $this->db->select('match_players.id as m_p_id');
        $this->db->from('match');
        $this->db->where('match.unique_id',$match_id);
        $this->db->where('players.pid',$player_id);
        $this->db->join('match_players', 'match_players.matchid = match.match_id');
        $this->db->join('players', 'players.id = match_players.playerid');
        return $this->db->get()->row_array();
    }
    
    function get_match_id_by_unique_id($id)
    {
        $this->db->select('match_id');  
        $this->db->where('unique_id',$id);
        $this->db->order_by('match_id','desc');
        $this->db->where('match_status !=','Cancelled');
        $this->db->limit('1');
        $resp =  $this->db->get('match')->row();
        if($resp !="")
        {
    ...
    function get_today_fixture_match()
    {
        $date = date("Y-m-d");
        $this->db->select('match_id,unique_id,match_date_time');
        $this->db->where('DATE(match_date_time)', $date);
     ...
     function my_match_record($request,$status,$limit)
    {
        $this->db->select('my_match.*,match.unique_id,match.match_date_time');
        $this->db->where('my_match.match_status',$status);
        $this->db->where('my_match.user_id',$request['user_id']);
        $this->db->group_by('my_match.match_id');
        if($status =="Result")
        {
            $this->db->order_by('match.match_date_time','desc');
        }   
        else
        {
            $this->db->order_by('match.match_date_time','asc');
        } 
        $this->db->limit($limit);
        $this->db->join('match', 'match.match_id = my_match.match_id');
        $resp =  $this->db->get('my_match')->result_array();
        if($resp !="")
        {
            return $resp;
        }    
    ...
    function check_countest_full($id)
    {
        $this->db->select('total_team,join_team');
        $this->db->where('contest_id',$id);
        $contest = $this->db->get('contest')->row_array();
        if($contest)
        {
            return $contest;
        }    
    ...
    function create_new_contest($contest)
    {
        $contest_data = array('contest_name' =>$contest->contest_name,
        ...
        if($last_id !="")
        {
            ...
            //$this->db->order_by('winning_info_id','asc');
            $Win_info =  $this->db->get('winning_information')->result_array();

            foreach ($Win_info as $info) {
                $Win_data = array('contest_id' =>$last_id,
                                'rank' =>$info['rank'],
                                'from_rank' =>$info['from_rank'],
                                'to_rank' =>$info['to_rank'],
                                'price' =>$info['price']
                            );
                $this->db->insert('winning_information',$Win_data);
            }
        }    
    }
    
    function get_team_id_in_match_table($team_uid,$match_id)
    {
        $this->db->select('teamid1');
        $this->db->where('teamid1',$team_uid);
        $this->db->where('match_id',$match_id);
    ...
    function get_teampoints_caluclation($user_id,$teamid)
    {
        $this->db->select('SUM(`total_points`) + SUM(`batting_points`) + SUM(`bolling_points`) + SUM(`fielding_points`) + SUM(`second_innings_batting`) + SUM(`second_innings_bolling`) + SUM(`second_innings_fielding`) as total_points');
        $this->db->where('user_id',$user_id);
        $this->db->where('my_team_id',$teamid);
        $resp = $this->db->get('my_team_player')->row()->total_points;
        if($resp)
    ...
    function get_leaderboard($contest_id,$team_id,$match_id,$user_id)
    {
        ...
        $this->db->where('matchid',$match_id);
        $this->db->where('user_id',$user_id);
        return $this->db->get('leaderboard')->row()->points;
    }
    
    // API for popup offer 
    function popup_offer()
    {   
        $this->db->select('*');
    ...
    function check_match_status($id)
    {
        $this->db->select('match_status');
        $this->db->where('match_id',$id);
        $data = $this->db->get('match')->row_array();
        if($data)
        {
            return $data;
        }    
    ...
    function checkCount($rank, $contestid)
    {
        $this->db->select('id');
        $this->db->where('contestid',$contestid);
    ...
    function checkLowerrCount($rank, $contestid,$tempRank)
    {
        $this->db->select('user_id,rank,points,id,temprank');
        $this->db->where('contestid',$contestid);
        $this->db->where('rank =',$rank);
        $this->db->where('temprank <',$tempRank);
        $resp = $this->db->get('leaderboard');
        if($resp->num_rows() > 0)
        {
            return $resp->result_array();
    ...
    function checkUpperCount($rank, $contestid,$tempRank)
    {
        $this->db->select('user_id,rank,points,id,temprank');
        $this->db->where('contestid',$contestid);
        $this->db->where('rank =',$rank);
    ...
    function get_Contestwinner($contest_id)
    {
        $this->db->select_max('to_rank');
        $this->db->where('contest_id',$contest_id);
        $resp = $this->db->get('winning_information')->row()->to_rank;
        if($resp)
        {   
            return $resp;
        }    
    ...
    function update_rankby_pointsNow($id , $rank)
    {
        $this->db->where('id',$id);
        $this->db->update('leaderboard',array('temprank' =>$rank));
    }

    function getRankAndPoint($lId, $MId, $CID, $points)
    {
        $sql = "SELECT `temprank`, `points`, `rank`, `id` FROM `leaderboard` WHERE `temprank` < '".$lId."' AND `points` = '".$points."'  AND `matchid` = '".$MId."' AND `contestid` = '".$CID."' ORDER BY `leaderboard`.`temprank` + 0 desc" ;
