...
class Webservice_model extends CI_Model 
{
    ...
    function contest_list($request)
    {
        if($request['type'] =="All")
        {
            $sql =  "select *,(total_team - join_team) as difference from contest where `match_id` ='".$request['match_id']."' AND `cancelled` ='0' order by difference asc";
...
WHERE contestid = '".$request['contest_id']."' 
    ...
    function add_amount($request,$txn_status)
    {
        ...
        if($id)
        {
            ...
            $check = $this->db->get('fix_bonus')->row_array();
            if( !empty($check))
            {
    ...
    {
        $this->db->select('pancard_status,aadharcard_status,bank_status');
        $this->db->where('user_id',$request['user_id']);
        $resp = $this->db->get('registration')->row();
        if($resp != "")
        {
   ...
   function get_match_player_id($match_id, $player_id)
    ...
    function upload_bank_proof($data , $user_id)
    {
        $this->db->where('user_id',$user_id);
        $update = $this->db->update('registration',$data);
        if($update)
